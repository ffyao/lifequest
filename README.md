...
- **好友与榜单**：支持按用户名单向添加好友，排行榜可切换总榜和好友榜，并支持分页查看。
...
- 普通激活码只能注册 1 个账号。
- 高级激活码生成时可以自定义可激活次数，范围为 2 到 10000。
- 高级激活码如果不填写次数，默认生成 300 次。
- 管理员可以废除已有高级激活码；废除后该激活码剩余次数变为 0，不能再用于注册。
...
- 普通激活码只能使用一次。
- 高级激活码可按管理员设置的次数重复使用。
- 管理员可以生成普通激活码和高级激活码、查看有效激活码状态，并废除已有高级激活码。

好友与排行榜说明：

- 好友管理是独立导航页面。
- 用户可以搜索用户名并单向添加好友，不需要对方确认。
- 好友榜展示当前用户自己和已添加好友。
- 排行榜支持总榜和好友榜切换。
- 排行榜分页加载，每页默认显示 20 条，避免一次加载过多用户数据。

## AI 任务生成规则

- 任务只能由 DeepSeek 生成，不使用本地模板兜底。
- 任务生成接口有 10 秒冷却时间，避免连续快速重复请求。
- 每个用户每天最多生成 30 次任务，超出后需次日再试。
